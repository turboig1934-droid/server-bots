...
Runner: ${this.runnerId}`;
    ...
    cmdCross(bot, args) {
        ...
        for (let i = 0; i < count; i++) {
            const t = i / count;
            const x = (bot.x || 0) + (t - 0.5) * 30;
            const z = (bot.z || 0) + (i % 2 === 0 ? 0 : (t - 0.5) * 20);
            const b = targets[i];
            if (b.connected) {
                b.chat(`/tp ${b.username} ${x} 65 ${z}`);
            }
        }
        bot.chat(`✝️ Cross formation!`);
    }
    ...
    cmdCount(bot, args) {
        const active = this.bots.filter(b => b.connected).length;
        const registered = this.bots.filter(b => b.registered).length;
        bot.chat(`📊 Total: ${this.bots.length} | Active: ${active} | Registered: ${registered} | Removed: ${this.totalRemoved}`);
    }
    ...
    cmdUptime(bot, args) {
        ...
        bot.chat(`⏱️ Uptime: ${hours}h ${minutes % 60}m ${seconds % 60}s`);
    }

    cmdMemory(bot, args) {
        const used = process.memoryUsage();
        bot.chat(`💾 Memory: ${Math.round(used.heapUsed / 1024 / 1024)}MB / ${Math.round(used.heapTotal / 1024 / 1024)}MB`);
    }
    ...
    cmdHelpAll(bot, args) {
        ...
        bot.chat(`📚 All commands: ${commands}`);
    }
    ...
    displayStatus() {
        ...
        const selected = this.selectedBots.size;
        const registered = this.bots.filter(b => b.registered).length;
        console.log(`📊 STATUS: ${active}/${this.bots.length} active | Selected: ${selected} | Registered: ${registered} | Runner: ${this.runnerId}`);
    }
    ...
    async runVideoHelper() {
        console.log('🎬 Video Helper Mode Activated');
        
        const sequences = [
            { cmd: 'army', msg: '🎥 Forming Army' },
            { cmd: 'circle', msg: '🎥 Circle Formation' },
            { cmd: 'formation', msg: '🎥 Battle Formation' },
            { cmd: 'attack', msg: '🎥 Attack!' },
            { cmd: 'dance', msg: '🎥 Dance Celebration' }
        ];
        ...
        console.log('✅ Video Helper Complete!');
    }
